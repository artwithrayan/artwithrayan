...
function getSiteContent() {
  ...
  try {
    ...
    if (content.bannerImages?.length || !content.aboutImage || content.aboutImage === "/images/aboutme.jpg" || oldAboutBodies.includes(content.aboutBody)) {
      const updated = {
        ...content,
        bannerImages: defaultSiteContent.bannerImages,
        aboutImage: !content.aboutImage || content.aboutImage === "/images/aboutme.jpg" ? defaultSiteContent.aboutImage : content.aboutImage,
...
ensureColumn("payments", "tracking_number", "TEXT");
ensureColumn("payments", "tracking_url", "TEXT");
...
function getPaymentByPrintfulOrderId(printfulOrderId) {
  return db.prepare(`
    SELECT * FROM payments
    WHERE printful_order_id=?
    ORDER BY created_at DESC
    LIMIT 1
  `).get(String(printfulOrderId));
}
...
function markPaymentTrackingEmailSent(paymentId) {
  db.prepare(`
    UPDATE payments
    SET tracking_email_sent_at=CURRENT_TIMESTAMP
    WHERE id=?
  `).run(paymentId);
}
